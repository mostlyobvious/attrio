  ...
  context "standard casting conventions" do
    let(:model) do
      Class.new do
        ...
        define_attributes { attr :integer_attribute, Integer }
      end
    end
    ...
    context "with not typecasted assignment" do
      ...
      it "should not cast an object which has not method to_i" do
        expect { object.integer_attribute = [] }.not_to raise_exception
  ...
  context ":base option passed" do
    let(:model) do
      Class.new do
        ...
        define_attributes { attr :integer_attribute, Integer, base: 16 }
      end
    end
